...
void GPS_NMEA::GPS_delay_ms(unsigned int t)
{
    delay_ms(t);
}

void GPS_NMEA::GPS_delay_us(unsigned int t)
{
    delay_us(t);
}
...
{
    Hands_Free_Usart_Init(GPS_USART,9600);
}
...
void GPS_NMEA::GPS_Usart_Send_Data(unsigned char* dbuf,unsigned short int len)
{
    ...
    {
        while(USART_GetFlagStatus(GPS_USART,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕
        USART_SendData(GPS_USART,dbuf[j]);
    }
}
